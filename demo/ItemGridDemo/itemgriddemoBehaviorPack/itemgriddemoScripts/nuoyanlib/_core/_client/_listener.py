...
def listen_engine_and_lib(self):
    ...
    for event in _ALL_CLIENT_ENGINE_EVENTS:
        if _is_method_overridden(self.__class__, NuoyanClientSystem, event):
            ...
            self.ListenForEvent(_CLIENT_ENGINE_NAMESPACE, _CLIENT_ENGINE_SYSTEM_NAME, event, self, method)
    for event, sys_name in _ALL_CLIENT_LIB_EVENTS.items():
        if _is_method_overridden(self.__class__, NuoyanClientSystem, event):
            method = getattr(self, event)
...
def listen_for_lib_sys(event):
    return listen_for(event, _LIB_NAME, _LIB_SERVER_NAME)
