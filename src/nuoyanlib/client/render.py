...
def set_query_mod_var(entity_id, name, value, sync=True):
    ...
    支持全局同步（即所有客户端同步设置该变量的值）。若不进行全局同步，则本次设置只对当前客户端有效。
    若设置的变量未注册，会自动进行注册。
...
def add_player_render_resources(player_id, rebuild, *res_tuple):
    ...
    使用本接口添加的资源的 identifier，需要遵循以下命名规范：
    ...
    :param bool rebuild: 是否重建玩家的数据渲染器，传入 True 时会自动调用 RebuildPlayerRender 接口
    :param tuple[str,str] res_tuple: [变长位置参数] 渲染资源元组，第一个元素为资源键名（短名称），第二个参数为具体资源名称（模型ID、贴图路径、动画名称等）

    :return: 返回添加结果（是否成功），结果为一个元组，元素类型为 bool，与 res_tuple 一一对应
    :rtype: tuple[bool]
    """
    res = []
...
def add_entity_render_resources(entity_id, rebuild, *res_tuple):
    ...
    :param str entity_id: 实体ID
    :param bool rebuild: 是否重建实体的数据渲染器，传入 True 时会自动调用 RebuildActorRender 接口
    :param tuple[str,str] res_tuple: [变长位置参数] 渲染资源元组，第一个元素为资源键名（短名称），第二个参数为具体资源名称（模型ID、贴图路径、动画名称等）
